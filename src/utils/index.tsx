...
export function splitValueText(text: string) {
  // Matches:
  // - USD 1.9T
  // - +18%
  // - <10%
  ...
  if (match) {
    return { mainValue: match[1].trim(), rest: match[2].trim() };
  }
  return { mainValue: null, rest: text };
}

export const buildMediaTypeAndSrc = (
  mediaSrcLink: string,
): { type: 'image' | 'video' | 'pdf' | 'unknown'; src: string | null } => {
  if (!mediaSrcLink) {
