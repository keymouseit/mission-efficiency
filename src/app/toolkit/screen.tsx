...
import { CONSTS, countries } from "@/constants";
import {
  createQueryString,
  findRegionFromISO,
  getAlpha2FromISO,
...
import CountryOverviewModal from "@/components/sections/CountryOverviewModal";
        ...
        async (position) => {
          ...
          try {
            ...
            if (data && (data.countryCode || data.countryName)) {
              ...
              if (data.countryName) {
                // prefetch the user country page
                router.prefetch(
                  `/toolkit/${slugify(data.countryName, { lower: true })}/dashboard`
                );
              }

              // data.countryCode is the Alpha-2 code (e.g. 'US') which matches
              // the countries list's `code` field. Use it to select the country.
              const countryCode = data.countryCode || "";
              handleCountrySelect(countryCode);

              // Center the map on the user's coordinates and open the tooltip
