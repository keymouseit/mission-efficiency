...
interface TrainingCardSliderProps {
  sliderData: DrupalNode[];
}

const TrainingCardSlider: React.FC<TrainingCardSliderProps> = ({
  sliderData,
}) => {
  const settings = {
    dots: true,
  ...
  return (
    <>
      <Slider {...settings} className="slider-theme-arrow">
        {sliderData?.map((trainingItem: DrupalNode, index: number) => {
          const sluggedLink = `trainings/${slugify(
            `${trainingItem.title} ${trainingItem.id}`
          )}`;
          const mediaTypeAndSrc = buildMediaTypeAndSrc(
            trainingItem.field_t_media_url
          );
          return (
            <div
              key={index}
                  ...
                  {mediaTypeAndSrc.type === "image" ? (
                    ...
                    <div className="w-full h-full bg-placeholder flex items-center justify-center">
                      {mediaTypeAndSrc.type === "pdf" ? (
                        ...
                        <>{renderIcon(trainingItem?.field_t_resource?.name)}</>
                      )}
                    </div>
                  )}
                </div>
                <div className="exactLaptop:w-[60%] exactLaptop:p-4 flex flex-col h-full w-full laptopMax:py-0.5">
                  <div className="w-full h-full flex flex-col justify-between pr-1 mobileMax:pr-0">
                    <div className="">
                      ...
                      {/* <div
												className="mobileMax:!hidden mb-2 flex items-start text-left lieExactTab:line-clamp-3 line-clamp-6 webkit-box text-small text-cardText leading-6 mobileMax:text-xsmall mobileMax:leading-normal"
                    ...
                    <div className="flex items-start flex-wrap">
                      {trainingItem?.field_t_resource?.name ? (
                        <p className="text-xsmall mobileMax:text-[13px] text-gray font-medium leading-6 --font-poppins pr-2 mobileMax:pr-1 mr-2 mobileMax:mr-2 border-r-[1px] border-gray flex items-center">
                          {formatDateToUS(trainingItem?.field_t_resource?.name)}
                        </p>
                      ) : null}
                      {trainingItem?.field_t_topic?.name ? (
...
export default TrainingCardSlider;
