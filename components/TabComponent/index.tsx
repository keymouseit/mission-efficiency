...
const TabComponent: React.FC<countryProps> = ({
  countryDetail,
  headerData,
  footerData,
}) => {
  ...
  const renderInvestTab = () => {
    const investText = typeof field_ec_invest_text === 'object' ? field_ec_invest_text?.value : field_ec_invest_text;
    return (
      <div className="py-10">
        <motion.div
  ...
  const renderSupportTab = () => {
    const supportText = typeof field_ec_support_text === 'object' ? field_ec_support_text?.value : field_ec_support_text;
    return (
      <div className="py-10">
        <motion.div
          className="country-list text-cardText text-medium leading-normal --font-poppins mobileMax:text-small"
        ...
        <div className="flex flex-wrap justify-start box-border pt-20">
          {field_support_resource_cards?.map((card: DrupalNode, index: number) => (
            <Link
              ...
              >
                <div className="h-full w-full">
                  <div className="flex justify-center items-center w-full h-[250px] betweenMdDesk:h-[200px] overflow-hidden relative">
                    {card?.field_link === "https://www.youtube.com/watch?v=0I_Wmhjc5Jo" ? (
                      <ReactPlayer
                        url={card?.field_link || ""}
  ...
  const renderElevateTab = () => {
    ...
    return (
      <div className="py-10">
        ...
        />
        <div className="flex flex-wrap justify-start box-border pt-20">
          {field_elevate_resource_cards?.map((card: DrupalNode, index: number) => (
            <Link
              href={card?.field_link || "#"}
              key={index}
              target="_blank"
